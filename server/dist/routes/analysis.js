...
router.get('/:id', auth_1.enforceTenantIsolation, async (req, res) => {
    try {
        ...
        const analysisResult = await database_1.pool.query(`SELECT a.*, u.email as created_by_email
       FROM site_analyses a
       JOIN site_users u ON a.created_by = u.id
       WHERE a.id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await database_1.pool.query(`INSERT INTO site_analyses (tenant_id, status, created_by, pricing_version_id)
       VALUES ($1, 'LIVE', $2, $3)
       RETURNING *`, [tenantId, userId, pricingVersionId]);
        // Audit log
...
router.put('/:id/inputs', auth_1.enforceTenantIsolation, async (req, res) => {
    try {
      ...
      ON CONFLICT (analysis_id) DO UPDATE SET
        ...
        const results = await calculationService_1.CalculationService.computeAnalysis(calculationInputs, pricingVersionId);
        // Upsert results
      ...
      ON CONFLICT (analysis_id) DO UPDATE SET
        ...
        await database_1.pool.query('UPDATE site_analyses SET updated_at = CURRENT_TIMESTAMP WHERE id = $1', [id]);
...
router.post('/:id/lock', auth_1.enforceTenantIsolation, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const tenantId = req.user.tenant_id;
       ...
       RETURNING *`, [id]);
        ...
        // Audit log
        await database_1.pool.query(`INSERT INTO site_audit_logs (user_id, tenant_id, action, target_type, target_id)
       VALUES ($1, $2, $3, $4, $5)`, [userId, tenantId, 'LOCK_ANALYSIS', 'analysis', id]);
...
router.post('/:id/unlock', auth_1.authenticate, async (req, res) => {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5)`, [userId, 'UNLOCK_ANALYSIS', 'analysis', id, JSON.stringify({ admin_action: true })]);
        res.json({ analysis: result.rows[0] });
    }
    catch (error) {
