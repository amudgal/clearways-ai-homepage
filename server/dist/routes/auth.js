...
router.post('/otp/send', async (req, res) => {
    try {
        ...
        const tenantResult = await database_1.pool.query('SELECT id, name FROM site_tenants WHERE domain = $1 AND status = $2', [domain, 'ACTIVE']);
        ...
        await database_1.pool.query(`INSERT INTO site_otp_store (email, code, expires_at, attempts)
       VALUES ($1, $2, $3, 0)
       ON CONFLICT (email) 
...
router.post('/otp/verify', async (req, res) => {
    try {
        ...
        const otpResult = await database_1.pool.query('SELECT code, expires_at, attempts FROM site_otp_store WHERE email = $1', [email]);
        ...
        // Verify OTP
        if (otpData.code !== otp) {
            await database_1.pool.query('UPDATE site_otp_store SET attempts = attempts + 1 WHERE email = $1', [email]);
        ...
        const domain = email.split('@')[1]?.toLowerCase();
        const tenantResult = await database_1.pool.query('SELECT id FROM site_tenants WHERE domain = $1 AND status = $2', [domain, 'ACTIVE']);
        if (tenantResult.rows.length === 0) {
        ...
        let userResult = await database_1.pool.query('SELECT id, email, tenant_id, role FROM site_users WHERE email = $1', [email]);
        let userId;
        if (userResult.rows.length === 0) {
            // Create new user
            const newUserResult = await database_1.pool.query(`INSERT INTO site_users (email, domain, tenant_id, role, last_login_at)
         VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)
         RETURNING id, email, tenant_id, role`, [email, domain, tenantId, role]);
            ...
            await database_1.pool.query('UPDATE site_users SET last_login_at = CURRENT_TIMESTAMP WHERE id = $1', [userId]);
        }
        // Generate JWT token
