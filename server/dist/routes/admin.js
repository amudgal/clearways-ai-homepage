...
router.get('/pricing/versions', async (req, res) => {
    try {
        const result = await database_1.pool.query(`SELECT pv.*, u.email as created_by_email
...
router.post('/pricing/versions', async (req, res) => {
    try {
        ...
        await database_1.pool.query('UPDATE site_pricing_versions SET is_active = false');
        const result = await database_1.pool.query(`INSERT INTO site_pricing_versions (version, effective_date, created_by, is_active)
       VALUES ($1, $2, $3, true)
       RETURNING *`, [versionName, effectiveDate, userId]);
        // Audit log
        await database_1.pool.query(`INSERT INTO site_audit_logs (user_id, action, target_type, target_id, metadata)
       VALUES ($1, $2, $3, $4, $5)`, [userId, 'CREATE_PRICING_VERSION', 'pricing_version', result.rows[0].id, JSON.stringify({ version: versionName })]);
...
router.get('/pricing/:versionId/:provider', async (req, res) => {
    try {
        ...
        const result = await database_1.pool.query(`SELECT id, service_type, tier, region, unit_type, unit_price, annual_multiplier, metadata
       FROM site_cloud_pricing
       WHERE pricing_version_id = $1 AND provider = $2
       ORDER BY service_type, tier`, [versionId, provider]);
...
router.post('/pricing', async (req, res) => {
    try {
        const { pricing_version_id, provider, service_type, tier, region, unit_type, unit_price, annual_multiplier, metadata, } = req.body;
        ...
        const updateResult = await database_1.pool.query(`UPDATE site_cloud_pricing
       SET unit_price = $1, 
           annual_multiplier = $2, 
           metadata = $3,
           tier = $4,
       ...
       RETURNING *`, [
        ...
        const result = await database_1.pool.query(`INSERT INTO site_cloud_pricing (
...
router.delete('/pricing/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await database_1.pool.query('DELETE FROM site_cloud_pricing WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Pricing entry not found' });
        }
        // Audit log
        await database_1.pool.query(`INSERT INTO site_audit_logs (user_id, action, target_type, target_id)
       VALUES ($1, $2, $3, $4)`, [req.user.id, 'DELETE_PRICING', 'cloud_pricing', id]);
...
router.get('/audit-logs', async (req, res) => {
    try {
        ...
        const result = await database_1.pool.query(`SELECT al.*, u.email as user_email, t.name as tenant_name
...
router.get('/tenants', async (req, res) => {
    try {
        const result = await database_1.pool.query(`SELECT t.*, 
...
router.post('/tenants', async (req, res) => {
    try {
        ...
        const existingResult = await database_1.pool.query('SELECT id FROM site_tenants WHERE domain = $1', [domain.toLowerCase()]);
        ...
        const result = await database_1.pool.query(`INSERT INTO site_tenants (name, domain, status)
       VALUES ($1, $2, $3)
       RETURNING *`, [name, domain.toLowerCase(), status]);
        // Audit log
        await database_1.pool.query(`INSERT INTO site_audit_logs (user_id, action, target_type, target_id, metadata)
       VALUES ($1, $2, $3, $4, $5)`, [
            userId,
...
router.put('/tenants/:id', async (req, res) => {
    try {
        ...
        const existingResult = await database_1.pool.query('SELECT * FROM site_tenants WHERE id = $1', [id]);
        if (existingResult.rows.length === 0) {
            return res.status(404).json({ error: 'Tenant not found' });
        }
        const updates = [];
        ...
        if (domain !== undefined) {
            ...
            const domainCheck = await database_1.pool.query('SELECT id FROM site_tenants WHERE domain = $1 AND id != $2', [domain.toLowerCase(), id]);
        ...
        params.push(id);
        const result = await database_1.pool.query(`UPDATE site_tenants 
...
router.delete('/tenants/:id', async (req, res) => {
    try {
       ...
       RETURNING *`, [id]);
        // Audit log
        await database_1.pool.query(`INSERT INTO site_audit_logs (user_id, action, target_type, target_id)
       VALUES ($1, $2, $3, $4)`, [userId, 'DELETE_TENANT', 'tenant', id]);
        res.json({ tenant: result.rows[0] });
    }
    catch (error) {
