...
router.delete('/analyses/tenant/:tenantId/versions', async (req: AuthRequest, res) => {
  try {
    ...
    // Verify tenant exists
    const tenantResult = await pool.query(
      'SELECT id, name FROM site_tenants WHERE id = $1',
      [tenantId]
    );
    ...
    const analysesResult = await pool.query(
      'SELECT id FROM site_analyses WHERE tenant_id = $1',
      [tenantId]
    );
    ...
    if (analysisIds.length > 0) {
      ...
      const deleteResult = await pool.query(
        `DELETE FROM site_analysis_versions 
         WHERE analysis_id = ANY($1::uuid[])
         RETURNING version_number, analysis_id`,
        [analysisIds]
      );
      ...
      await pool.query(
        'UPDATE site_analyses SET current_version_number = 0 WHERE tenant_id = $1',
        [tenantId]
      );
    }
...
router.put('/analyses/:id', async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    ...
    const analysisCheck = await pool.query(
      'SELECT id, title, status, tenant_id FROM site_analyses WHERE id = $1',
      [id]
    );
...
router.delete('/analyses/:id', async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const adminUserId = req.user!.id;
    ...
    const analysisCheck = await pool.query(
      'SELECT id, title, tenant_id FROM site_analyses WHERE id = $1',
      [id]
    );
    ...
    await pool.query('DELETE FROM site_analyses WHERE id = $1', [id]);
