...
async function deleteAmexVersions() {
  try {
    ...
    const tenantResult = await pool.query(
      "SELECT id, name FROM site_tenants WHERE LOWER(name) LIKE '%amex%' OR LOWER(domain) LIKE '%amex%'"
    );

    if (tenantResult.rows.length === 0) {
      console.error('Amex tenant not found. Available tenants:');
      const allTenants = await pool.query('SELECT id, name, domain FROM site_tenants');
    ...
    const analysesResult = await pool.query(
      'SELECT id, title, analysis_type, current_version_number FROM site_analyses WHERE tenant_id = $1',
      [amexTenant.id]
    );
    ...
    const updateResult = await pool.query(
      'UPDATE site_analyses SET current_version_number = 0 WHERE tenant_id = $1 RETURNING id, title',
      [amexTenant.id]
    );
