...
export default function AnalysisForm() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // Add MicroStrategy Architecture Diagram page before Insights
      try {
        pdf.addPage();
        ...
        // Title for Architecture Diagram
        pdf.setFontSize(18);
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(23, 162, 184); // #17A2B8
        ...
        // Create an iframe to load the architecture diagram
        const iframe = document.createElement('iframe');
        iframe.src = archUrl;
        iframe.style.width = '1216px'; // Standard width for PDF
        ...
        // Wait for iframe to load, then try to capture it
        await new Promise((resolve) => {
          iframe.onload = () => {
            setTimeout(resolve, 2000); // Give extra time for content to render
          };
          iframe.onerror = () => {
        ...
        try {
          ...
          if (iframeDoc && iframeDoc.body) {
            ...
            const archImgHeight = (archCanvas.height * archImgWidth) / archCanvas.width;
            
            // Check if image fits on page
            if (archYPos + archImgHeight > pageHeight - margin - 15) {
              // Image too large, scale it down
              const maxHeight = pageHeight - archYPos - margin - 15;
              const scaledWidth = (archCanvas.width * maxHeight) / archCanvas.height;
              pdf.addImage(archImgData, 'PNG', margin, archYPos, scaledWidth, maxHeight);
            } else {
              pdf.addImage(archImgData, 'PNG', margin, archYPos, archImgWidth, archImgHeight);
            }
          }
        } catch (e) {
        ...
        console.warn('Error adding architecture diagram:', error);
        // Continue with export even if architecture diagram fails
      }
